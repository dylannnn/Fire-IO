app.factory('PostsFactory', function($firebaseObject, $firebaseArray) {
    ...
    return {
        getPosts: function(page, fn) {
            ...
            var posts = $firebaseArray(new Firebase(config.fb + '/content/posts').orderByChild('created').startAt(postsStart).limit(config.posts.loadPerPage)).$loaded().then(function(posts) {
                // Put the page number into all of the posts
